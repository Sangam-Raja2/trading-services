...
public interface TradeRepository extends JpaRepository<Trade, Long> {
	...
	@Query( "from Trade  WHERE symbol=?1  AND type=?2 AND "
			+ " timestamp BETWEEN ?3 AND ?4")
	List<Trade> findBySymbolandType(String symbol, String type,  Date startDate,
			Date endDate);

	@Query(value = "SELECT MIN(price) from Trade WHERE symbol=?1 AND timestamp BETWEEN ?2 AND ?3")
	public Double findByMinPriceSymbol(String symbol,  Date startDate,
			 Date endDate);
        
        @Query(value = "SELECT MAX(price) from Trade WHERE symbol=?1 AND timestamp BETWEEN ?2 AND ?3")
